...
library(recipes)
USArrests.rec <- recipe(index~., data = USArrests) %>%  
  step_center(all_numeric(), -index) %>%  
  step_scale(all_numeric(), -index)
prep(USArrests.rec, training = USArrests) -> USArrests.pr
bake(USArrests.pr, new_data = USArrests) -> USArrests.ck

str(USArrests.ck)

USArrests.ck[, -5] %>%  princomp(., cor=TRUE) -> pca3
pca3 
pca3 %>% summary()
pca3 %>%  biplot()
